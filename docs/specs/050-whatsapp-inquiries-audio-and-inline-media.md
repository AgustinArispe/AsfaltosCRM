...
- CRM-051 supersedes the new-contact-only lifecycle clauses below: after CRM-051, any
  uniquely resolved Customer with no active `WHATSAPP` Opportunity receives a new
  WhatsApp Opportunity from the next valid inbound message, including an existing
  Customer or conversation. Active Opportunities from other sources do not block it.
  CRM-050 remains authoritative for inquiry/media representation.
