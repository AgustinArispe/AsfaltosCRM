    ...
    <article aria-labelledby={`opportunity-customer-${opportunity.id}`}>
      ...
      >
        <div className="space-y-3">
          <section aria-labelledby={`customer-information-${opportunity.id}`} className="ui-panel px-4 py-4 sm:px-5">
            <h3 className="text-sm font-semibold text-slate-950" id={`customer-information-${opportunity.id}`}>
              ...
              <div>
                ...
                <dd className="mt-0.5 break-words text-sm text-slate-900">
                  {opportunity.customer.email ? (
                    <a className="underline decoration-slate-300 underline-offset-2 hover:decoration-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500" href={`mailto:${opportunity.customer.email}`}>
                      {opportunity.customer.email}
                    </a>
                  ) : (
              ...
              <div>
                ...
                <dd className="mt-0.5 text-sm text-slate-900">
                  {opportunity.customer.phone ? (
                    <a className="underline decoration-slate-300 underline-offset-2 hover:decoration-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500" href={`tel:${opportunity.customer.phone}`}>
                      {opportunity.customer.phone}
                    </a>
                  ) : (
                    <MissingValue>No informado</MissingValue>
          ...
          <section aria-labelledby={`quote-${opportunity.id}`} className="ui-panel px-4 py-4 sm:px-5">
                ...
                >
                  <thead>
                    <tr className="border-b border-slate-200 text-xs text-slate-500">
                      <th className="pb-2 font-medium" scope="col">Producto</th>
                      <th className="pb-2 text-right font-medium" scope="col">Cantidad</th>
                    </tr>
                  </thead>
                  <tbody>
                    {opportunity.products.map((quotedProduct) => (
                      <tr className="border-b border-slate-100 last:border-b-0" key={quotedProduct.product.id}>
        ...
        <section aria-labelledby={`history-${opportunity.id}`} className="ui-panel px-4 py-4 sm:px-5">
          <h3 className="text-sm font-semibold text-slate-950" id={`history-${opportunity.id}`}>
            Historial
          </h3>
          <ol className="mt-3">
            {opportunity.history.map((entry, index) => (
              <li className="relative grid grid-cols-[0.75rem_minmax(0,1fr)] gap-3 pb-4 last:pb-0" key={entry.id}>
