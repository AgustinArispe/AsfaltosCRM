...
function ContactDetails({ opportunity }: { opportunity: OpportunityDetail }) {
  return (
    <>
      <div>
...
function LossReason({ opportunity }: { opportunity: OpportunityDetail }) {
  if (opportunity.status !== 'PERDIDA' || !opportunity.loss_reason) return null

  return (
    <div className='opportunity-detail__loss-reason'>
      <span className='text-xs font-medium text-[var(--destructive-text)]'>Motivo de pérdida</span>
...
export function OpportunityDetailModalHeader({
  opportunity,
  returnAffordance,
  titleId,
}: {
  opportunity: OpportunityDetail | null
  returnAffordance: ReactNode
  titleId: string
}) {
  return (
...
function DrawerHeader({ opportunity }: { opportunity: OpportunityDetail }) {
  return (
    <header className='opportunity-detail__header px-4 py-4 sm:px-5'>
...
function CustomerInformation({ opportunity }: { opportunity: OpportunityDetail }) {
  return (
    <section
...
function Quote({ opportunity }: { opportunity: OpportunityDetail }) {
...
function History({ opportunity }: { opportunity: OpportunityDetail }) {
  return (
    <section
...
export function OpportunityDetailContent({
  ...
  const contextContent = contextual ?? <History opportunity={opportunity} />

  return (
    <article
      aria-labelledby={isDrawer ? `opportunity-customer-${opportunity.id}` : undefined}
      className={`opportunity-detail ${isDrawer ? 'opportunity-detail--drawer' : 'opportunity-detail--page'}`}
    >
      {isDrawer ? (
        <DrawerHeader opportunity={opportunity} />
      ) : (
        <div className='opportunity-detail__overview'>
      ...
      <div
        className={
          isDrawer
            ...
            : 'opportunity-detail__body opportunity-detail__body--page'
        }
      >
        <div
          className={`opportunity-detail__primary ${isDrawer ? '' : 'opportunity-detail__primary--page'}`}
        >
          {isDrawer ? <CustomerInformation opportunity={opportunity} /> : null}
          <Consultation opportunity={opportunity} />
        ...
        {isDrawer ? (
          ...
          <aside aria-label='Actividad y notas' className='opportunity-detail__context'>
            {contextContent}
          </aside>
        )}
      </div>
