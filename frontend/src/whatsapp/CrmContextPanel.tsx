...
function ContextField({ label, value }: { label: string; value: string | null }) {
  return (
    <div>
      <dt className="text-[0.6875rem] font-bold uppercase tracking-wide text-slate-500">
        {label}
      </dt>
...
export function CrmContextPanel({
  ...
  const confirmationTitle = confirmation?.kind === 'unlink'
  ...
  const confirm = async () => {
    ...
    await onUpdateLink(
      confirmation.kind === 'link' ? confirmation.opportunity.id : null,
    )
  ...
  return (
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
        {conversation.resolution_status === 'NEEDS_REVIEW' ? (
          <div className="rounded-[4px] border border-rose-200 bg-rose-50 px-3 py-3" role="status">
            ...
            <p className="mt-1 text-xs leading-5 text-rose-800">
              El teléfono coincide de forma ambigua. No se puede responder ni vincular
              una oportunidad hasta resolverlo.
            </p>
          </div>
        ) : null}
        ...
        <section aria-labelledby="context-opportunity-title" className="mt-5 border-t border-slate-200 pt-4">
            ...
            <div className="mt-4">
              ...
              <ul className="mt-2 space-y-2">
                {conversation.opportunity_suggestions.map((suggestion) => (
                  <li className="flex items-center justify-between gap-2 rounded-[4px] border border-slate-200 px-3 py-2" key={suggestion.id}>
          ...
          <div className="mt-5 flex justify-end gap-2">
            <Button
              data-modal-initial-focus={
                confirmation?.kind === 'unlink' ? true : undefined
              }
              disabled={isLinking}
            ...
            <Button
              data-modal-initial-focus={
                confirmation?.kind === 'link' ? true : undefined
              }
              disabled={isLinking}
