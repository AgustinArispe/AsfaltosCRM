...
export function ChatPanel({
  ...
  if (detailStatus === 'error' || !conversation) {
    return (
      <section className="grid min-h-0 flex-1 place-items-center bg-white px-5 text-center" aria-label="Chat activo">
        <div>
          <p className="text-sm font-semibold text-slate-900">
            {detailError ? 'No pudimos abrir la conversación' : 'Seleccioná una conversación'}
          </p>
          <p className="mt-1 max-w-sm text-xs leading-5 text-slate-500">
            {detailError ?? 'Elegí un contacto de la lista para ver sus mensajes y contexto comercial.'}
          </p>
          {detailError ? (
  ...
  return (
    ...
    >
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          >
            <svg aria-hidden="true" className="size-5" fill="none" viewBox="0 0 24 24">
              <path d="m15 5-7 7 7 7" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.7" />
            </svg>
          </button>
      ...
      {!isOnline ? (
        <div className="shrink-0 border-b border-amber-200 bg-amber-50 px-4 py-2 text-xs font-medium text-amber-900" role="status">
      ...
      {conversation.template_required ? (
        <div className="shrink-0 border-b border-slate-300 bg-slate-100 px-4 py-2 text-xs text-slate-700" role="status">
