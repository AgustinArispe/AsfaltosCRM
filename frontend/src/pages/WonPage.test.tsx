...
describe('WonPage', () => {
  ...
  it('filters, paginates, opens detail, and resets a populated history', async () => {
    ...
    render(<WonPage />)
    expect(await screen.findAllByText('Vial Sur SA')).toHaveLength(2)
    ...
    const openButton = screen.getAllByRole('button', { name: 'Abrir oportunidad' })[0]
    if (!openButton) throw new Error('Missing won detail action')
  ...
  it('offers retry after a recoverable initial failure', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Sin resultados')).toBeVisible())
  })
})
